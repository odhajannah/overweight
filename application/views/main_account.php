		...
		<div class="modal fade" id="tambah" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalSizeLg" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
				<div class="modal-content">
					...
					<form action="<?=base_url()?>main_account/addData" enctype="multipart/form-data" method="POST">
						<input type="text" id="tipe" name="tipe" hidden>
						<input type="text" id="id_number" name="id_main" hidden >
						<div class="modal-body">
							<div class="form-group">
								<label for="exampleInputPassword1">Number
								<span class="text-danger">*</span></label>
								<input type="text" class="form-control" id="number" name="number" placeholder="Number Code" />
							</div>
							<div class="form-group">
	...
	<script type="text/javascript">
		"use strict";
			var table = $('#kt_datatable').DataTable({ 
				...
				"ajax": {
					"url": "<?php echo site_url('main_account/get_data_user')?>",
					"type": "POST"
				},
				columns: [
                    {data: 'no'},
			...
			function edit_data(id_number) {
				...
				$.ajax({
					url: '<?php echo base_url() ?>main_account/ajax_edit/'+id_number,
					type: "POST",
					dataType: "JSON",
					cache: false,
					success: function(data){
						console.log(data);
						$('#id_number').val(data.id_main);
						$('#number').val(data.id_number);
						$('#nama').val(data.nama);
						$('#tipe').val('update');
						$('#tambah').modal({
							"backdrop"  : "static",
			...
			$("#deletedata").click(function(){
				...
				$(location).attr('href', '<?php echo base_url('main_account/ajax_delete')?>/'+stat5+'')
